...
namespace WeixinTookeen.Client.Services
{
    public class LoginService
    {
        ...
        {
            byte[] bytes = HttpServer.SendGetRequest(_session_id_url);
            _session_id = Encoding.UTF8.GetString(bytes).Split(new string[] { "\"" }, StringSplitOptions.None)[1];
            bytes = HttpServer.SendGetRequest(_qrcode_url + _session_id);
        ...
        public object LoginCheck()
        {
            ...
            byte[] bytes = HttpServer.SendGetRequest(_login_check_url + _session_id);
        ...
        {
            byte[] bytes = HttpServer.SendGetRequest(login_redirect + "&fun=new&version=v2&lang=zh_CN");
